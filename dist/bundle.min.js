...
export async function sendFileOverDC(dc, file, keys, roomStorageId){
  ...
  if(window.__CIPHERNEXUS_AES_KEY && dc && dc.readyState === 'open'){
    try{
      const payload = await encryptBlobForTransfer(file);
      dc.send(payload);
      return fileId;
    }catch(e){
...
window.CN_FileTransfer = window.CN_FileTransfer || (function(){
  ...
  async function sendFile(dc, file, meta={}){
    ...
    while(offset<total){ const slice = file.slice(offset, offset+CHUNK); const ab = await slice.arrayBuffer(); const enc = await encryptChunk(window.__CIPHERNEXUS_AES_KEY, new Uint8Array(ab)); dc.send(JSON.stringify({type:'file-chunk', id, offset, iv:enc.iv, ct:enc.ct})); offset += CHUNK;
  ...
  async function handleIncoming(obj){
    try{
      ...
      else if(obj.type==='file-end'){ const id=obj.id; const parts = incoming[id].buf; const totalLen = parts.reduce((s,p)=>s+p.length,0); const u8 = new Uint8Array(totalLen); let pos=0; parts.forEach(p=>{ u8.set(p,pos); pos+=p.length; }); const blob = new Blob([u8]);
...
function base64UrlEncode(bytes){ const b64 = btoa(String.fromCharCode(...new Uint8Array(bytes))); return b64.replace(/\+/g,'-').replace(/\
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  if(deriveBtn && roomInput){
    deriveBtn.addEventListener('click', async ()=>{
      ...
      const saltB64u = saltB64 ? saltB64.replace(/\+/g,'-').replace(/\
...
import argon2 from '.argon2lib and uncomment the local import below.
...
function subscribeRealtime(roomStorageId, handlers){
...
var LZString={decompressFromUTF16:function(x){return x;}};
...
window.jsQR = function(data, width, height){ return null; };
